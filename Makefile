...
.PHONY: new-chart
new-chart: ## Create a new chart from Copier template (usage: make new-chart DEST=charts/apps/my-app)
	@test -n "$(DEST)" || (echo "DEST is not set. Usage: make new-chart DEST=charts/apps/my-app"; exit 1)
...
.PHONY: update-chart
update-chart: ## Update a chart from the Copier template (usage: make update-chart DEST=charts/apps/my-app)
	@test -n "$(DEST)" || (echo "DEST is not set. Usage: make update-chart DEST=charts/apps/my-app"; exit 1)
	@./tools/update-charts.sh $(DEST)

.PHONY: list-managed-charts
list-managed-charts: ## List all Copier-managed charts
	@./tools/update-charts.sh --list

.PHONY: update-all-charts
update-all-charts: ## Update all Copier-managed charts using saved answers (usage: make update-all-charts)
