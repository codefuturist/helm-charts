# Testing Guide for pgAdmin Helm Chart

This guide covers all testing approaches for the pgAdmin Helm chart, from development to CI/CD integration.
...
```bash
helm lint . --set pgadmin.email=admin@example.com --set pgadmin.password=test123
```
...
helm install my-pgadmin . \
  --set pgadmin.email=admin@example.com \
  --set pgadmin.password=SuperSecure123 \
  --namespace pgadmin \
  --create-namespace

# Run Helm tests
helm test my-pgadmin -n pgadmin

# Clean up
helm uninstall my-pgadmin -n pgadmin
```
...
| `07-pgadmin-features.yaml` | SMTP, pgpass, config_local |
...
.PHONY: test-pgadmin
test-pgadmin:
    cd charts/pgadmin && ./test.sh

.PHONY: validate-pgadmin
validate-pgadmin:
    cd charts/pgadmin && ./validate.sh

.PHONY: lint-pgadmin
lint-pgadmin:
    helm lint charts/pgadmin \
        --set pgadmin.email=admin@example.com \
        --set pgadmin.password=test123
```
