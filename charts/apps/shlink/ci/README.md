# Shlink Chart CI Tests

This directory contains CI test configurations for the Shlink Helm chart. These tests are automatically run by Chart Testing (ct) to validate the chart before release.

## Test Configurations

| File                             | Purpose                | Key Features Tested                         |
| -------------------------------- | ---------------------- | ------------------------------------------- |
| `01-default-values.yaml`         | Default configuration  | Basic deployment with PostgreSQL subchart   |
...
| `04-server-definitions.yaml`     | Web client setup       | Pre-configured server connections           |
...
| `07-pgadmin-features.yaml`       | Shlink-specific        | GeoLite, Redis, security settings           |
| `08-ingress-enabled.yaml`        | Dual ingress           | Backend and web client ingress with TLS     |
...
| `11-container-config.yaml`       | Advanced config        | External database, Redis, advanced settings |
...
- Default configuration with PostgreSQL
...
- Server configuration via ConfigMap
- Dual ingress setup (backend + webclient)
- Independent scaling

### Database Tests

- Embedded PostgreSQL subchart
...
1. Create a new YAML file in this directory: `NN-test-name.yaml`
2. Add test-specific configuration values
3. Update this README with test description
4. Run locally to validate:
   ```bash
   helm template test ../. -f NN-test-name.yaml
   ```

## Notes

- All tests use `postgresql.enabled: true` by default unless testing external databases
- Tests use minimal resources for faster CI execution
- Persistence is disabled by default to avoid PVC cleanup issues in CI
- Some tests may require specific Kubernetes features (e.g., NetworkPolicy support)
