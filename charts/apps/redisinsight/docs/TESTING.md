# Testing Guide for RedisInsight Helm Chart

This guide covers all testing approaches for the RedisInsight Helm chart, from development to CI/CD integration.
...
```bash
helm lint . --set redisinsight.email=admin@example.com --set redisinsight.password=test123
```
...
helm template test . \
  ...
  --set redisinsight.password=test123
...
helm install my-redisinsight . \
  --set redisinsight.email=admin@example.com \
  --set redisinsight.password=SuperSecure123 \
  --namespace redisinsight \
  --create-namespace

# Run Helm tests
helm test my-redisinsight -n redisinsight

# Clean up
helm uninstall my-redisinsight -n redisinsight
```
...
```bash
# Lint all charts
ct lint --charts charts/redisinsight

# Install and test
ct install --charts charts/redisinsight
```
  ...
  lint-test:
    ...
    steps:
      ...
      - name: Run chart-testing (lint)
        run: ct lint --charts charts/redisinsight

      - name: Create kind cluster
        uses: helm/kind-action@v1

      - name: Run chart-testing (install)
        run: ct install --charts charts/redisinsight
```
...
```makefile
.PHONY: test-redisinsight
test-redisinsight:
    cd charts/redisinsight && ./test.sh

.PHONY: validate-redisinsight
validate-redisinsight:
    cd charts/redisinsight && ./validate.sh

.PHONY: lint-redisinsight
lint-redisinsight:
    helm lint charts/redisinsight \
        --set redisinsight.email=admin@example.com \
        --set redisinsight.password=test123
```
...
```
================================================
RedisInsight Helm Chart Test Suite
================================================
