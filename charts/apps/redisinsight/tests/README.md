# RedisInsight Chart Unit Tests

This directory contains unit tests for the RedisInsight Helm chart using [helm-unittest](https://github.com/helm-unittest/helm-unittest).
...
helm unittest charts/redisinsight
```
...
- name: Run Helm Unit Tests
  run: |
    helm plugin install https://github.com/helm-unittest/helm-unittest
    helm unittest charts/redisinsight --color
```
...
helm-unittest:
  ...
  script:
    - helm plugin install https://github.com/helm-unittest/helm-unittest
    - helm unittest charts/redisinsight --output-type JUnit --output-file test-results.xml
  artifacts:
    reports:
...
```
### Chart [ redisinsight ] charts/redisinsight

 PASS  test redisinsight common labels and metadata tests/common_test.yaml
 PASS  test redisinsight deployment tests/deployment_test.yaml
 PASS  test redisinsight statefulset    tests/statefulset_test.yaml
 PASS  test redisinsight service    tests/service_test.yaml
 PASS  test redisinsight secrets    tests/secret_test.yaml
 PASS  test redisinsight configmaps tests/configmap_test.yaml
 PASS  test redisinsight ingress    tests/ingress_test.yaml
 PASS  test redisinsight monitoring tests/monitoring_test.yaml
 PASS  test redisinsight network policy tests/networkpolicy_test.yaml
 PASS  test redisinsight RBAC   tests/rbac_test.yaml
 PASS  test redisinsight PVC and PDB    tests/pvc_test.yaml
 PASS  test redisinsight diagnostic mode    tests/diagnostic_test.yaml
...
helm template test . --set redisinsight.email=admin@example.com --set redisinsight.password=test123 --show-only templates/deployment.yaml
```
