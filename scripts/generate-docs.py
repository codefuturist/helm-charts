...
def generate_chart_doc(chart_path: Path, output_dir: Path) -> Optional[List[ValueEntry]]:
    ...
    if output_file.exists() and output_file.read_text() == doc_content:
        return entries
...
def generate_values_index(all_entries: List[ValueEntry], output_dir: Path):
    ...
    json_content = json.dumps(index_data, indent=2)
    _write_if_changed(json_file, json_content)
...
"""
    ...
    for entry in sorted_entries:
        chart_link = f"[{entry.chart}](../charts/{entry.chart}.md#{entry.anchor_id})"
        content += f"| {chart_link} | `{entry.key}` | {entry.type} | {entry.section} |\n"
