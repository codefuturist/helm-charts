...
# Custom YAML loader that ignores unknown tags (e.g. !python/name: in mkdocs.yml)
...
class ChartDocsSync:
    ...
    def update_mkdocs_nav(self, charts: Dict[str, Path]) -> bool:
        ...
        Uses the custom loader for reading (to handle MkDocs YAML tags like
        !python/name:) and text-level replacement for writing (to preserve
        formatting, comments, and custom tags that yaml.dump would destroy).
        """
        if not self.mkdocs_config.exists():
            print("⚠️  Warning: mkdocs.yml not found, skipping nav update")
            return False

        raw_text = self.mkdocs_config.read_text()

        config = yaml.load(raw_text, Loader=_SafeLoaderIgnoreUnknown)
        ...
        # Build the replacement YAML snippet for the Charts nav section
        lines = ["  - Charts:"]
        for item in new_charts_nav:
        ...
        # Replace the Charts section in the raw text using regex
        import re as _re
        ...
        new_text, count = pattern.subn(new_section + "\n", raw_text)

        if count == 0:
            print("⚠️  Warning: Could not locate Charts nav section in mkdocs.yml")
            return False

        self.mkdocs_config.write_text(new_text)
    ...
    def update_charts_index(self, charts: Dict[str, Path]) -> bool:
        ...
        for info in chart_info:
            table_lines.append(f"| [{info['name']}]({info['name']}.md) | {info['version']} | {info['description']} |")
        ...
        if generate_docs and available_charts:
            ...
            try:
                ...
                for chart_name, chart_path in available_charts.items():
                    entries = generate_docs_module.generate_chart_doc(chart_path, self.charts_docs_dir)
                    if entries:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sync Helm charts with MkDocs documentation")
    parser.add_argument(
        "--no-generate",
        action="store_true",
