...
## Migrating an Existing Chart to Copier Management

To bring an existing chart under Copier management (so it receives template updates
automatically via git hooks and `update-charts.sh`):

### Step 1 — Create `.copier-answers.yml`

Create a `.copier-answers.yml` in the chart root with answers matching the chart's
current configuration. Example for a chart named `my-app`:

```yaml
# Changes here will be overwritten by Copier; DO NOT EDIT MANUALLY
_commit: ~
_src_path: /path/to/repo/templates/chart-copier
app_version: "1.0.0"
chart_description: My application chart
chart_name: my-app
chart_version: "0.1.0"
container_port: 8080
...
./tools/update-charts.sh --defaults charts/apps/my-app
# Or:
just chart-recopy-all
```

### Step 3 — Clean up orphaned files

Copier **does not delete** files that it didn't generate. If you disabled a feature
(e.g., `use_hpa: false`) but the chart had a pre-existing `hpa.yaml`, you must
remove it manually:

```bash
# Check for templates that reference disabled values
helm lint charts/apps/my-app
# Remove any orphaned templates that cause lint errors
rm charts/apps/my-app/templates/hpa.yaml
```
...
Once a chart has `.copier-answers.yml`, it will be automatically regenerated by the
`copier-recopy` pre-commit hook whenever files in `templates/chart-copier/` change.
Install hooks with:
